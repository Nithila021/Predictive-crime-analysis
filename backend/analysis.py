...
from models import predict_dbscan_cluster, predict_demographic_zone, get_crime_density_classification, predict_crime_zone
...
def analyze_safety(lat, lon):
    ...
    # Get crime zone
    try:
        zone = predict_crime_zone(lat, lon)
        if zone is None:
            zone = 0  # Default value if prediction fails
    except Exception as e:
        print(f"Error predicting crime zone: {str(e)}")
        # Provide a default zone
        zone = 0
