...
def predict_crime_zone(lat, lon, crime_type=None):
    """
    Predict crime zone for a new point based on location and optionally crime type
    
    Args:
        lat (float): Latitude
        lon (float): Longitude
        crime_type (str, optional): Type of crime. If None, nearest neighbor approach is used.
        
    Returns:
        int: Predicted crime zone
    """
    if cached_data.get('crime_clusters') is None:
        print("Crime clusters not initialized")
        return None
    
    # Extract cached components
    crime_kmeans = cached_data['crime_clusters']
    
    # If we don't have a crime type, use the nearest neighbor approach
    if crime_type is None:
        # Get the dataframe with crime zones
        df = cached_data['df']
        ...
        nearest = df.iloc[df['temp_distance'].idxmin()]
        
        # Return its zone
        return nearest['crime_zone']
    else:
        # We have a crime type, so we can use the actual model prediction
        # Get the encoder and scaler from cache
        if 'crime_encoder' not in cached_data or 'crime_scaler' not in cached_data:
            # Store these in cache during initialization if not already stored
            print("Crime encoder or scaler not found in cache")
            return None
        
        crime_encoder = cached_data['crime_encoder']
        crime_scaler = cached_data['crime_scaler']
        
        # Create feature vector
        coords = np.array([[lat, lon]])
        
        # One-hot encode the crime type
        crime_df = pd.DataFrame([{'crime_type': crime_type}])
        try:
            crime_encoded = crime_encoder.transform(crime_df[['crime_type']])
        except (ValueError, KeyError):
            # Handle unknown crime type
            print(f"Unknown crime type: {crime_type}")
            return predict_crime_zone(lat, lon, None)  # Fall back to nearest neighbor
            
        # Combine features
        features = np.hstack([coords, crime_encoded])
        
        # Scale features
        features_scaled = crime_scaler.transform(features)
        
        # Predict zone
        return crime_kmeans.predict(features_scaled)[0]